// @ts-nocheck — illustrative example; not part of the build.
...
async function processStaleNotifications(): Promise<void> {
  ...
  await withSpan('cleanup-stale-notifications', async (span) => {
    ...
    const staleNotifications = await withSpan('db.query.stale-notifications', async (innerSpan) => {
    ...
    for (const notification of staleNotifications) {
      await withSpan('process-notification', async (innerSpan) => {
